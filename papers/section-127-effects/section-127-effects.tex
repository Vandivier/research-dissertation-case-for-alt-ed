...
% are the VAR structural or just structural-like due to cholesky ordering?
% I don't use `svar` command so I think just structural-like
% articles below on var, svar, impulse-response and oirf using Cholesky decompositon
%   I still think Cholesky decomp is structural-like, not structural, but it can get policy effect measure
% ref:
%   [if you are going to read one, make it this, my main equation source ->] https://blog.stata.com/2016/09/20/structural-vector-autoregression-models/
%   https://www.youtube.com/watch?v=wqwDcY9pq5I&t=769s
%   https://www.youtube.com/watch?v=PJp3317BdHo
%   https://www.youtube.com/watch?v=iV8_nQoB9CU
%   https://faculty.washington.edu/ezivot/econ584/notes/varModels.pdf
%   https://online.stat.psu.edu/stat510/lesson/11/11.2#paragraph--329
%   https://blog.stata.com/2016/02/18/vector-autoregressionsimulation-estimation-and-inference-in-stata/
...
%   https://fmwww.bc.edu/EC-C/S2013/823/EC823.S2013.nn10.slides.pdf
\begin{subequations}
    \begin{equation}
...
% cite for Cholesky ordering https://blog.stata.com/2016/09/20/structural-vector-autoregression-models/
...
\begin{subequations}
    ...
    %     = Cy_t + e_t
    %     = Ay_t
    %     % = C_1y_{t-1} + ... + C_iy_{t-i} + e_t
    %     \label{eq:var_contemporaneous}
    % \end{equation}
    \begin{equation}
        e_t = Bu_t
    ...
    %     = A^{-1} B B' {A^{-1}}'
    %     = B B'
    %     \label{eq:structural_}
    % \end{equation}
    % \begin{equation}
    %     \Sigma = E(e_te_t')
    %     = A^{-1} B B' {A^{-1}}'
    %     = B B'
    %     \label{eq:reduce_using_error_covariance}
    % \end{equation}
    \begin{equation}
        \Sigma = E(e_t e_t')
        = E(Bu_tu_t'B')
...
% Equation \ref{eq:var_contemporaneous} simplifies \ref{eq:applied_naiive_var}
% by abstracting $k$.
% $C$ is a matrix of coefficients over time.
% $A$ is a matrix of coefficients with embedded error.
% Equation {eq:make_structural_shocks} is a definition,
% and it allows
