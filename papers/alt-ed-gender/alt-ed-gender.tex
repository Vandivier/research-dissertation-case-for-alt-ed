...
Male identification is retained in a second-best regularized model ($ \lambda_1 / \lambda_2 = 0.019, k = 57, r2=0.40 $).
...
but does so with substantially fewer variables ($ \lambda_1 / \lambda_2 = 0.05, k = 34, r2=0.41 $).

When interactions are included, many gender interactions are included in the preferred model.
One gender interaction is included in a strict model associated with heavy factor penalization.
This heavily penalized model contains five factors, representing the top two percent of factors according to this measure of factor importance.
...
Two kinds of univariate selection and two kinds of SFS
...
% This elastic net algorithm is tuned by checking many values o f
% % Whether the respondent is male is the only gender variable in the first pool of one hundred and sixty-five variables.
